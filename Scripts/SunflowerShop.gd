...
func _ready():
	...
	$Control/HBoxContainer/VBoxContainer/FoxPrice.text = str(Global.sunflowerprice)

func camera_offset(new_position):
	camera_of = int(new_position.x)

func _on_mouse_entered():
	if Global.coincount >= Global.sunflowerprice and Global.decorationcount < Global.treecount * 10:
...
func _process(delta):
	if dragging and Global.dropable:
		sprite.global_position = Vector2((get_global_mouse_position() - of).x, 373)
		sprite.visible = true
	if dragging and !Global.dropable:
...
func _on_texture_button_button_up():
	if dragging == true:
		if sprite.visible == true:
			...
			Global.coincount -= Global.sunflowerprice

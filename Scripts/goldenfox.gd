...
func _ready():
	...
	$AnimatedSprite2D.scale.x = (randi() & 2) - 1
	...
	if !mannequin:
		...
		if process:
			...
			if actor.global_position.x < -40:
				process = false
				queue_free()
			var velocity = -Global.walk_speedf
			actor.position += Vector2(velocity, 0)
...
func start_wandering():
	...
	if Global.treecount == 0:
		...
		target_position = Vector2(randf_range(200, max(Global.treecount * Global.x_spacing + 300, Global.foxcount * 400)), actor.position.y)
